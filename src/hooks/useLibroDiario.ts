...
export const useLibroDiario = ({ empresaId, activeView }: UseLibroDiarioProps) => {
  ...
  const incorrectTxsCount = useMemo(() => {
    return filteredTransactions.filter(tx => {
      ...
      // 2. Asientos de cobros/pagos de tesorería que utilicen la cuenta madre/genérica 2.1.1, 2.1 o similar
      ...
      if (esTesoreria) {
        const hasMotherAccount = codes.includes('2.1.1') || codes.includes('2.1') || codes.includes('2');
        if (hasMotherAccount) return true;
      }
      return false;
    }).length;
  }, [filteredTransactions]);
      ...
      for (const tx of incorrectTxs) {
        ...
        // B) Caso de asientos de tesorería con cuentas madre o incorrectas como 2.1.1 en la contrapartida
          ...
          if (movsTesoreria && movsTesoreria.length > 0 && movsTesoreria[0].id_documento) {
            ...
            if (correctAccId) {
              const counterMov = tx.movimientos.find(m => isPago ? m.debe > 0 : m.haber > 0);
